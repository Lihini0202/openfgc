...
public class AdminNomineeController {
    ...
    public AdminNomineeController(NominationService nominationService) {
    ...
    @PreAuthorize("hasAuthority('SCOPE_portal:profile:read:any')")
    public List<NominationResponse> pending() {
        return nominationService.getPendingActivation().stream().map(NominationResponse::from).toList();
    }
