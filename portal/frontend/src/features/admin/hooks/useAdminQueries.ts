...
/** The owner has no nomination at all - not an error, just an empty state. */
export function useNominationByOwnerQuery(
  ownerId: string | undefined,
): UseQueryResult<NominationResponse | null> {
  return useQuery<NominationResponse | null>({
    queryKey: ['admin', 'nomination-by-owner', ownerId],
    queryFn: async (): Promise<NominationResponse | null> => {
      try {
        return await fetchNominationByOwner(String(ownerId))
      } catch (error) {
...
> {
  ...
  return useMutation({
    mutationFn: async ({ nominationId, reason }: DeactivateVariables): Promise<NominationResponse> =>
