...
func registerServices(mux *http.ServeMux, log *slog.Logger, cfg config.Config) error {
	...
	// Registered without authManager on purpose: every /acting/* route
	// authenticates with the impersonation token rather than the caller's own,
	// so it verifies that token itself and re-checks the live nomination on
	// every request.
