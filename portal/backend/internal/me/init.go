...
func Initialize(mux *http.ServeMux, cfg config.Config, authManager *auth.Manager) error {
	...
	mux.Handle("GET /me/session", authManager.Require(http.HandlerFunc(handler.Session)))
	...
	mux.Handle("PUT /me/consents/{consentId}/revoke", authManager.Require(http.HandlerFunc(handler.ConsentRevoke), auth.ScopeConsentsWriteSelf))
