...
func Initialize(mux *http.ServeMux, cfg config.Config, authManager *auth.Manager) error {
	...
	// Approving carries its own scope. Revoking withdraws processing the user
	// already chose; approving authorises new processing, and a nominee granted
	// one must not thereby hold the other.
	mux.Handle("POST /me/consents/{consentId}/approve", authManager.Require(http.HandlerFunc(handler.ConsentApprove), auth.ScopeConsentsApproveSelf))
