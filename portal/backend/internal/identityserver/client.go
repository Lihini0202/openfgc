...
// findByAttribute returns users whose attribute exactly equals value, using
// SCIM2's server-side `filter` query.
func (c *Client) findByAttribute(ctx context.Context, attribute, value string) ([]User, error) {
	escaped := strings.ReplaceAll(value, `"`, `\"`)
	query := url.Values{}
	query.Set("filter", fmt.Sprintf(`%s eq "%s"`, c.filterPathFor(attribute), escaped))
...
func (c *Client) FindByNomineeID(ctx context.Context, nomineeID string) ([]User, error) {
	return c.findByAttribute(ctx, "nominee_id", nomineeID)
}
...
func (c *Client) FindByEmail(ctx context.Context, email string) (*User, error) {
	users, err := c.findByAttribute(ctx, "email", email)
	if err != nil {
		return nil, err
	}
	for i := range users {
...
func (c *Client) Search(ctx context.Context, query string) ([]User, error) {
	...
	for _, attribute := range []string{"email", "username"} {
		users, err := c.findByAttribute(ctx, attribute, query)
		if err != nil {
			return nil, err
		}
		for i := range users {
