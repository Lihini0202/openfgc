...
type Config struct {
	...
	IdentityServer IdentityServerConfig `koanf:"identityserver"`
...
// IS is the sole identity system: it issues login tokens (this config) and holds the
// user directory (IdentityServerConfig's SCIM2 settings below).
...
// IdentityServerConfig contains connection settings for the real WSO2 Identity Server
// instance used only for minting nominee impersonation tokens. This is a separate
// system from ThunderID, which remains the everyday login IdP for all users.
type IdentityServerConfig struct {
	BaseURL            string        `koanf:"base_url"`
	ClientID           string        `koanf:"client_id"`
	ClientSecret       string        `koanf:"client_secret"`
	ImpersonationScope string        `koanf:"impersonation_scope"`
	Timeout            time.Duration `koanf:"timeout"`
	TLSSkipVerify      bool          `koanf:"tls_skip_verify"`

	// IssuerURL and MaskAudience are used to verify impersonation ("mask")
	// tokens against IS's JWKS. Both fall back to the login-side auth config
	// when empty, since the mask token is issued by the same IS instance to the
	// same client as the portal's own login tokens. Override only when a
	// deployment separates them.
	IssuerURL    string `koanf:"issuer_url"`
	MaskAudience string `koanf:"mask_audience"`

	// RedirectURI is where IS returns the subject_token fragment after the
	// impersonation authorize step. It must be registered on the IS application
	// and is normally a frontend route, since only the browser can read a
	// URL fragment.
	RedirectURI string `koanf:"redirect_uri"`

	// Directory (SCIM2) settings - IS is the sole identity system, covering
	// both login and the user directory.
	DirectoryScopes string `koanf:"directory_scopes"`
	// CustomSchemaURN is the SCIM2 schema extension URN under which our
	// application-specific attributes (nominee_id, nominee_activated, etc.)
	// are nested. Must be defined as custom local claims mapped into this
	// SCIM2 dialect in IS's admin console before these attributes will
	// actually appear on User resources.
	CustomSchemaURN string `koanf:"custom_schema_urn"`
	AdminRoleName   string `koanf:"admin_role_name"`
}

// InternalConfig contains settings for service-to-service endpoints called by
// Nominee Service rather than by the frontend, and for the reverse direction:
// the BFF's own calls to Nominee Service's nomination gate.
type InternalConfig struct {
	// NomineeServiceURL is the base URL of Nominee Service, which owns the
	// nomination gate and the per-nominee permission set.
	NomineeServiceURL string `koanf:"nominee_service_url"`
	// GateAPIKey authenticates the BFF to Nominee Service's /internal/nominations/*
	// endpoints. Must match Nominee Service's impersonation-gate.internal-api-key.
	GateAPIKey string `koanf:"gate_api_key"`
	// GateTimeout bounds the live permission check that runs on every acting
	// request. Kept short: it is on the request path.
	GateTimeout time.Duration `koanf:"gate_timeout"`
}
...
func setDefaults(k *koanf.Koanf) error {
	...
	if err := k.Set("identityserver.base_url", ""); err != nil {
		return err
	}
	if err := k.Set("identityserver.client_id", ""); err != nil {
		return err
	}
	if err := k.Set("identityserver.client_secret", ""); err != nil {
		return err
	}
	if err := k.Set("identityserver.impersonation_scope", "internal_user_impersonate"); err != nil {
		return err
	}
	if err := k.Set("identityserver.timeout", "10s"); err != nil {
		return err
	}
	if err := k.Set("identityserver.tls_skip_verify", false); err != nil {
		return err
	}
	if err := k.Set("identityserver.directory_scopes", "internal_user_mgt_view internal_user_mgt_list internal_user_mgt_update internal_user_mgt_create internal_role_mgt_view internal_group_mgt_view"); err != nil {
		return err
	}
	if err := k.Set("identityserver.custom_schema_urn", "urn:scim:wso2:schema"); err != nil {
		return err
	}
	if err := k.Set("identityserver.admin_role_name", "PortalAdmin"); err != nil {
		return err
	}
	if err := k.Set("identityserver.issuer_url", ""); err != nil {
		return err
	}
	if err := k.Set("identityserver.mask_audience", ""); err != nil {
		return err
	}
	if err := k.Set("identityserver.redirect_uri", ""); err != nil {
		return err
	}
	if err := k.Set("internal.nominee_service_url", ""); err != nil {
		return err
	}
	if err := k.Set("internal.gate_api_key", ""); err != nil {
		return err
	}
	if err := k.Set("internal.gate_timeout", "3s"); err != nil {
		return err
	}
	if err := setAuthDefaults(k); err != nil {
		return err
	}
...
func setAuthDefaults(k *koanf.Koanf) error {
	...
	if err := k.Set("auth.tls_skip_verify", false); err != nil {
		return err
	}
	return nil
}
...
func validate(cfg Config) error {
	...
	if err := validateIdentityServer(cfg.IdentityServer); err != nil {
		return err
	}
	if err := validateImpersonation(cfg); err != nil {
		return err
	}
...
// knownDevSecrets are placeholder values published in this repository. They
// are rejected outright in production so a deployment cannot inherit a
// secret that is public knowledge.
var knownDevSecrets = map[string]struct{}{
	"dev-impersonation-gate-key": {},
	"dev-local-only-key":         {},
	"changeme":                   {},
}

// validateImpersonation enforces that the nominee impersonation path is either
// fully configured or fully off. There is no partial mode: a half-configured
// gate would silently fall back to a hardcoded key, producing a working
// shared secret from a missing environment variable.
func validateImpersonation(cfg Config) error {
	if strings.TrimSpace(cfg.IdentityServer.BaseURL) == "" {
		// Impersonation is not wired up. The acting routes fail closed at
		// request time; nothing to validate here.
		return nil
	}

	if strings.TrimSpace(cfg.Internal.NomineeServiceURL) == "" {
		return fmt.Errorf("internal.nominee_service_url must be set when identityserver.base_url is configured")
	}
	if strings.TrimSpace(cfg.Internal.GateAPIKey) == "" {
		return fmt.Errorf("internal.gate_api_key must be set when identityserver.base_url is configured")
	}
	if cfg.Internal.GateTimeout <= 0 {
		return fmt.Errorf("internal.gate_timeout must be > 0")
	}

	gateURL, err := url.Parse(strings.TrimSpace(cfg.Internal.NomineeServiceURL))
	if err != nil {
		return fmt.Errorf("internal.nominee_service_url must be a valid URL: %w", err)
	}
	if gateURL.Scheme != "http" && gateURL.Scheme != "https" {
		return fmt.Errorf("internal.nominee_service_url must use http or https scheme")
	}

	// Mask tokens cannot be verified without an issuer to discover JWKS from.
	// Rather than let the acting routes 502 forever, refuse to start.
	if strings.TrimSpace(cfg.IdentityServer.IssuerURL) == "" &&
		strings.TrimSpace(cfg.Auth.IssuerURL) == "" {
		return fmt.Errorf(
			"identityserver.issuer_url must be set when identityserver.base_url is configured " +
				"(no auth.issuer_url to fall back to); mask tokens cannot be verified without it")
	}

	if strings.EqualFold(cfg.Env, "production") {
		for name, secret := range map[string]string{
			"internal.gate_api_key": cfg.Internal.GateAPIKey,
		} {
			if _, isDev := knownDevSecrets[strings.TrimSpace(secret)]; isDev {
				return fmt.Errorf("%s must not use a known development value in production", name)
			}
		}
		if strings.TrimSpace(cfg.IdentityServer.MaskAudience) == "" &&
			strings.TrimSpace(cfg.Auth.ResourceAudience) == "" {
			return fmt.Errorf(
				"identityserver.mask_audience must be set in production; " +
					"without it the mask token audience check is skipped")
		}
		if cfg.IdentityServer.TLSSkipVerify {
			return fmt.Errorf("identityserver.tls_skip_verify must be false in production")
		}
	}
...
// validateAuth is skipped entirely when auth.enabled is false (the default), so
// existing placeholder-identity-based dev/test workflows are unaffected until
// real OIDC login is explicitly turned on.
...
// validateIdentityServer is intentionally lenient: an empty base_url just
// means impersonation isn't wired up, not a misconfiguration.
func validateIdentityServer(cfg IdentityServerConfig) error {
	if strings.TrimSpace(cfg.BaseURL) == "" {
		return nil
	}
	parsed, err := url.Parse(cfg.BaseURL)
	if err != nil {
		return fmt.Errorf("identityserver.base_url must be a valid URL: %w", err)
	}
	if parsed.Scheme != "http" && parsed.Scheme != "https" {
		return fmt.Errorf("identityserver.base_url must use http or https scheme")
	}
	if cfg.Timeout <= 0 {
		return fmt.Errorf("identityserver.timeout must be > 0")
	}
	if strings.TrimSpace(cfg.CustomSchemaURN) == "" {
		return fmt.Errorf("identityserver.custom_schema_urn must not be empty")
	}
	if strings.TrimSpace(cfg.AdminRoleName) == "" {
		return fmt.Errorf("identityserver.admin_role_name must not be empty")
	}
	return nil
}
