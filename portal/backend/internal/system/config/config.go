...
// IdentityServerConfig contains what delegated access and directory lookups need
// from the identity server.
//
// ClientID, ClientSecret and IssuerURL are not configured here. They are taken
// from AuthConfig, because the impersonation token is issued by the same
// identity server, to the same client, as the portal's own login tokens. Two
// independently configured copies would eventually disagree, and the resulting
// failure - tokens that verify for login but not for delegation - is a slow one
// to diagnose.
//
// RedirectURI is the exception and is configured below: login and delegation
// return to different places.
type IdentityServerConfig struct {
	// BaseURL is the identity server root, from which the authorize, token and
	// SCIM2 endpoints are built. AuthConfig.IssuerURL names the issuer rather
	// than the host, so it cannot stand in for this.
	BaseURL string `koanf:"base_url"`

	// MaskAudience is the audience expected in an impersonation token. Empty
	// falls back to ClientID, which is correct unless a deployment separates them.
	MaskAudience string `koanf:"mask_audience"`

	// ImpersonationScope is the built-in scope the identity server requires
	// before it will mint a subject token at all.
	ImpersonationScope string `koanf:"impersonation_scope"`

	// AdminRoleName is the role checked when deciding whether a user may review
	// nominations. Read per request, so a role change applies immediately.
	AdminRoleName string `koanf:"admin_role_name"`

	// CustomSchemaURN namespaces the identity server's custom user attributes,
	// which is where a nominee's own identifiers live.
	CustomSchemaURN string `koanf:"custom_schema_urn"`

	// DirectoryScopes are requested for the client-credentials token used to
	// look users up, as a single space-separated OAuth scope string. Separate
	// from the user-facing scopes: this token acts as the service, not as any
	// person.
	...
	// RedirectURI is where the identity server returns the subject_token
	// fragment. It is NOT the login redirect: login returns to the backend's
	// own callback, while the subject token must reach the browser, because the
	// fragment is readable only there. Configured separately for that reason.
	RedirectURI string `koanf:"redirect_uri"`

	// Derived from AuthConfig; never set from configuration.
	ClientID     string `koanf:"-"`
	ClientSecret string `koanf:"-"`
	IssuerURL    string `koanf:"-"`
}

// InternalConfig contains service-to-service settings for Nominee Service, which
// owns the nomination record and answers the gate.
type InternalConfig struct {
	// NomineeServiceURL is the base URL of Nominee Service.
	NomineeServiceURL string `koanf:"nominee_service_url"`

	// GateAPIKey authenticates this service to Nominee Service's
	// /internal/nominations/* endpoints. Must match Nominee Service's
	// impersonation-gate.internal-api-key. There is no default: a missing value
	// must fail at startup rather than silently produce a working secret.
...
type AuthConfig struct {
	...
	// Delegated-access cookies. They are named here, beside the login cookies,
	// because signing out must destroy every credential the browser holds - a
	// delegated token left behind after logout is still a working credential for
	// somebody else's account.
	ActingTokenCookie             string        `koanf:"acting_token_cookie"`
	ActingStateCookie             string        `koanf:"acting_state_cookie"`
	...
	// TLSSkipVerify disables certificate verification when talking to the
	// identity server. For local development only, where the identity server
	// serves a self-signed certificate: OIDC discovery happens at startup, so
	// without this the service cannot boot at all against one.
	//
	// Defaults to false. Anything but a developer's machine must leave it there.
	TLSSkipVerify bool `koanf:"tls_skip_verify"`
}
...
func Load() (*Config, error) {
	...
	// Delegation talks to the same identity server, as the same client, as login
	// does. Copying rather than configuring twice means the two cannot drift into
	// disagreement - a state where login works and delegation quietly does not.
	cfg.IdentityServer.ClientID = cfg.Auth.ClientID
	cfg.IdentityServer.ClientSecret = cfg.Auth.ClientSecret
	cfg.IdentityServer.IssuerURL = cfg.Auth.IssuerURL
	if strings.TrimSpace(cfg.IdentityServer.MaskAudience) == "" {
		cfg.IdentityServer.MaskAudience = cfg.Auth.ClientID
	}
...
func setDefaults(k *koanf.Koanf) error {
	...
	if err := k.Set("auth.acting_token_cookie", "portal-acting-token"); err != nil {
		return err
	}
	if err := k.Set("auth.acting_state_cookie", "portal-acting-state"); err != nil {
		return err
	}
	if err := k.Set("identity_server.base_url", "https://localhost:9443"); err != nil {
		return err
	}
	if err := k.Set("identity_server.redirect_uri", "http://localhost:5173/acting/callback"); err != nil {
		return err
	}
	if err := k.Set("identity_server.timeout", "10s"); err != nil {
		return err
	}
	if err := k.Set("identity_server.tls_skip_verify", false); err != nil {
		return err
	}
	if err := k.Set("identity_server.impersonation_scope", "internal_user_impersonate"); err != nil {
		return err
	}
	if err := k.Set("identity_server.admin_role_name", "PortalAdmin"); err != nil {
		return err
	}
	if err := k.Set("internal.nominee_service_url", "http://localhost:8082"); err != nil {
		return err
	}
	// No default for internal.gate_api_key on purpose: a deployment that forgets
	// it must fail rather than fall back to a shared secret everyone knows.
