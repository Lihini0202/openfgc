...
// CallerAccessToken reconstructs the caller's own access token from the split
// pair: part 1 arrives as a Bearer header, part 2 as an HttpOnly cookie.
//
// Exported because the impersonation flow needs it as the RFC 8693 `actor_token`
// — Identity Server proves the caller really is the nominee named in the subject
// token's `may_act` claim by comparing that token's subject against it.
