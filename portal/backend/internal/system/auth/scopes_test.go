...
func TestCanonicalScopesAreUniqueAndPrefixed(t *testing.T) {
	...
	if len(seen) != 14 {
		t.Fatalf("expected fourteen canonical scopes, got %d", len(seen))
	}
}
	...
	for route, rawPathItem := range paths {
		...
		for method, rawOperation := range pathItem {
			...
			case strings.HasPrefix(route, "/me/"):
				switch {
				// Approving is not a kind of writing: revoking withdraws
				// processing the user already chose, approving authorises new
				// processing, so the two are granted separately.
				case strings.HasSuffix(route, "/approve"):
					want = ScopeConsentsApproveSelf
				case upperMethod == "GET":
...
// its subject, so ":self" resolves to the owner's own data - precisely the
// boundary a nominee must stay inside. An :any scope here would let a nominee
// reach beyond the one owner their nomination concerns.
func TestDelegatableScopesAreSelfOnly(t *testing.T) {
	if len(DelegatableScopes) == 0 {
		t.Fatal("DelegatableScopes must not be empty")
	}
	for _, scope := range DelegatableScopes {
		if !strings.HasSuffix(scope, ":self") {
			t.Errorf("delegatable scope %q is not a :self scope", scope)
		}
	}
}

// Administrative reach must never be delegated. It is DelegatableScopes,
// not the canonical registry, that draws this line.
func TestAdministrativeScopesAreNotDelegatable(t *testing.T) {
	administrative := []string{
		ScopeProfileReadAny, ScopeProfileWriteAny,
		ScopeConsentsReadAny, ScopeConsentsWriteAny,
	}
	for _, admin := range administrative {
		for _, scope := range DelegatableScopes {
			if scope == admin {
				t.Errorf("administrative scope %q must not be delegatable", admin)
			}
		}
	}
}
