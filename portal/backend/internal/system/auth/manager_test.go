...
// owner granted the nominee, so a scope check alone cannot tell it apart from
// the owner's own token. Accepting one on a first-party route would attribute
// the nominee's actions to the owner, and would keep working after the owner
// revoked the nomination, since only the acting routes re-check it.
func TestDelegatedDetectsBothDelegationClaims(t *testing.T) {
	claim := func(raw string) map[string]json.RawMessage {
	...
	tests := []struct {
		...
		{"an ordinary token", `{"sub":"owner-uuid","scope":"portal:consents:read:self"}`, false},
		{"an exchanged delegated token", `{"sub":"owner-uuid","act":{"sub":"nominee-uuid"}}`, true},
		{"a subject token", `{"sub":"owner-uuid","may_act":{"sub":"nominee-uuid"}}`, true},
		{"an explicitly null claim", `{"sub":"owner-uuid","act":null}`, false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := delegated(claim(tt.claims)); got != tt.want {
