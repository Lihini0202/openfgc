...
const (
	...
	// Profile scopes: read/write cover normal edits; delete is kept separate
	// since it's the sensitive one, same reasoning as ACCOUNT_DELETE being
	// flagged "risky" in the nominee permission catalog.
	...
	// and vice versa.
	//
	// Deliberately excluded from AllPortalScopes: dev/placeholder identity must
	// never silently acquire cross-user access.
	ScopeProfileReadAny  = ScopePrefix + "profile:read:any"
	ScopeProfileWriteAny = ScopePrefix + "profile:write:any"
)

// Delegation rides on :self. A nominee's impersonation token carries the token
// subject's own scopes, with `sub` set to the owner - so ":self" resolves to the
// owner's data. No :any scope is ever delegatable, which is what bounds a
// nominee to the owner's own records and nothing wider.

// AllPortalScopes lists every canonical portal authorization scope granted to
// the placeholder/dev identity. The administrative :any profile scopes are
...
// are requested at mint time and then trimmed by the nomination validator to
// whatever the owner actually granted that nominee.
//
// Nothing administrative appears here, and nothing that would let a nominee
// manage nominations: delegation must not be transitive.
