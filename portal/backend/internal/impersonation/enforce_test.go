...
// A subject token is not an access token. It carries may_act ("this actor MAY
// act as sub"), reaches the browser in a URL fragment where scripts and history
// can see it, and is only turned into a credential by the exchange - which is
// where the identity server proves the caller really is the nominee. Accepting
// one here would let anyone who obtained it exercise the whole grant without
// ever holding the nominee's session.
func TestMaskVerifierRejectsSubjectTokenPresentedAsMask(t *testing.T) {
	is := newTestIS(t)

	subjectToken := is.validMask()
	subjectToken.Act = nil
	subjectToken.MayAct = &actor{Sub: "nominee-uuid"}

	if _, err := is.verifier().Verify(context.Background(), is.sign(t, subjectToken)); err == nil {
		t.Fatal("expected a may_act-only token to be refused on the acting path")
	}
}
...
func TestMaskTokenFromPrefersCookie(t *testing.T) {
	...
	if got := maskTokenFrom(r, actingTokenCookie); got != "cookie-token" {
...
func TestMaskTokenFromFallsBackToHeader(t *testing.T) {
	...
	if got := maskTokenFrom(r, actingTokenCookie); got != "header-token" {
...
func TestMaskTokenFromIgnoresNonBearer(t *testing.T) {
	...
	if got := maskTokenFrom(r, actingTokenCookie); got != "" {
