...
// nominee returns the delegated actor from whichever claim IS populated.
func (p maskPayload) nominee() string {
	if sub := strings.TrimSpace(p.Act.Sub); sub != "" {
		return sub
	}
	return strings.TrimSpace(p.MayAct.Sub)
}
...
// go-oidc), plus the presence of both `sub` and a delegation claim (`act` or
// `may_act`). A token carrying neither is not an impersonation token and is
// rejected outright - it is never treated as an ordinary token belonging to
// `sub`, which would silently hand the caller the owner's whole account.
