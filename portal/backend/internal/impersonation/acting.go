...
const (
	...
	// actingOwnerHeader lets a caller state which owner it believes it is acting
	// for, so a tab holding a superseded session is told rather than shown the
	// wrong person's records. See enforce.
	actingOwnerHeader = "X-Acting-Owner"
)
...
func (h *Handler) StartActing(w http.ResponseWriter, r *http.Request) {
	...
	h.setCookie(w, r, h.authCfg.ActingStateCookie, state+"|"+ownerID, actingStateMaxAge)
...
func (h *Handler) ExchangeActing(w http.ResponseWriter, r *http.Request) {
	...
	if subtle.ConstantTimeCompare([]byte(expectedState), []byte(strings.TrimSpace(req.State))) != 1 {
		h.clearCookie(w, r, h.authCfg.ActingStateCookie)
	...
	if err != nil {
		...
		h.audit.record(r.Context(), auditEvent{
			OwnerID: ownerID, NomineeID: nomineeFromToken(subjectToken),
			Event: eventSessionDenied, Detail: "exchange failed",
		})
		writeISError(w, err)
		return
	}
	...
	if err != nil {
		...
		h.audit.record(r.Context(), auditEvent{
			OwnerID: ownerID, NomineeID: nomineeFromToken(subjectToken),
			Event: eventSessionDenied, Detail: "exchanged token failed verification",
		})
	...
	if ownerID != "" && mask.Owner != ownerID {
		...
		h.audit.record(r.Context(), auditEvent{
			OwnerID: ownerID, NomineeID: mask.Nominee,
			Event: eventSessionDenied, Detail: "session mismatch",
		})
	...
	h.setCookie(w, r, h.authCfg.ActingTokenCookie, token.AccessToken, ttl)

	h.audit.record(r.Context(), auditEvent{
		OwnerID: mask.Owner, NomineeID: mask.Nominee, Event: eventSessionStarted,
		Detail: "scopes=" + strings.Join(scopeList(mask.Scopes), ","),
	})
...
func (h *Handler) StopActing(w http.ResponseWriter, r *http.Request) {
	h.clearCookie(w, r, h.authCfg.ActingTokenCookie)
	h.clearCookie(w, r, h.authCfg.ActingStateCookie)
...
func (h *Handler) readStateCookie(r *http.Request) (state, ownerID string, err error) {
	cookie, err := r.Cookie(h.authCfg.ActingStateCookie)
...
// nomineeFromToken reads the nominee from an unverified subject token, for audit
// only. A denied session still has to name who attempted it, and by definition
// the token could not be verified - so the value is recorded, never trusted.
func nomineeFromToken(raw string) string {
