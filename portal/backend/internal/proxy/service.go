...
func (s *Service) setTrustedHeaders(incoming *http.Request, outgoing *http.Request, trustedClientID string) {
	// org-id is a trusted header: it decides whose tenant the upstream serves,
	// so it may only ever come from a validated principal, never from the
	// client. A principal in context wins over the placeholder, which exists
	// only for local development.
	if principal, ok := systemcontext.PrincipalFromContext(incoming.Context()); ok {
		outgoing.Header.Set("org-id", principal.OrgID)
	} else if s.cfg.PlaceholderOrgID != "" {
